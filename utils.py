...
def load_sanders_data_from_twitter(dirname=".", line_count=-1):
    count = 0
...
def load_sanders_data(dirname=".", line_count=-1):
    ...
    with open(os.path.join(DATA_DIR, dirname, "full_corpus.csv"), "r") as csvfile:
        metareader = csv.reader(csvfile, delimiter=',', quotechar='"')
        for line in metareader:
            count += 1
            if line_count > 0 and count > line_count:
            ...
            topic, label, tweet_id, date, tweet = line
